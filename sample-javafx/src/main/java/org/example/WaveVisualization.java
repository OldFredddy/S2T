...
public class WaveVisualization extends WaveFormPane {
	...
	public WaveVisualization(int width, int height,double WavLen1,WavPlayer rec, Controller contr) {
		...
		//здесь таск на распознавание и таск на создание временного .wav

		/*
		setOnMouseClicked(m -> this.setMouseXPosition((int) m.getX()));
		setOnMouseClicked(m -> this.setTimerXPosition((int) m.getX()));
		//setOnMouseClicked(secondaryButtonDown -> stopPainterService());
